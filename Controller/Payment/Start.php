...
class Start extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        try {
            $transaction = $this->_gatewayClient->transactions()->create($this->_gatewayClient->getSiteId(), (int) round($order->getBaseGrandTotal() * 100), $order->getBaseCurrencyCode());
            
            $code = $order->getPayment()
                ->getMethodInstance()
                ->getCode();
            $paymentMethod = substr($code, 9);
            $transaction->setPaymentMethod($this->_gatewayClient->methods()
                ->get($paymentMethod));
            
            $transaction->setCallbackUrl($this->_url->escape($this->_url->getUrl('cardgate/payment/callback')));
            $transaction->setRedirectUrl($this->_url->escape($this->_url->getUrl('cardgate/payment/redirect')));
            ...
            foreach ($order->getAllVisibleItems() as $item) {
                ...
                $cartItem = $cart->addItem(\cardgate\api\Item::TYPE_PRODUCT, $item->getSku(), $item->getName(), $itemQty, round($item->getPrice() * 100, 0), $url);
                ...
                $stockData = NULL;
                try {
                    $stockData = $stock->get($item->getProduct()
                        ->getId())
                        ->getData();
                } catch (\Exception $e) { /* ignore */
                }
                if (is_array($stockData) && isset($stockData['manage_stock']) && isset($stockData['qty']) && ! ! $stockData['manage_stock']) {
            ...
            if ($shippingAmount > 0) {
                $cartItem = $cart->addItem(\cardgate\api\Item::TYPE_SHIPPING, 'shipping', 'Shipping Costs', 1, round($order->getShippingInclTax() * 100, 0));
            ...
            if ($discountAmount < 0) {
                $cartItem = $cart->addItem(\cardgate\api\Item::TYPE_DISCOUNT, 'discount', 'Discount', 1, round($discountAmount * 100, 0));
            ...
            if ($cardGateFeeAmount > 0) {
                $cartItem = $cart->addItem(\cardgate\api\Item::TYPE_HANDLING, 'cardgatefee', 'Payment Fee', 1, round($order->getCardgatefeeInclTax() * 100, 0));
            ...
            if ($calculatedVatTotal != $order->getTaxAmount()) {
                ...
                $cartItem = $cart->addItem(\cardgate\api\Item::TYPE_VAT_CORRECTION, 'cg-vatcorrection', 'VAT Correction', 1, round($vatCorrection * 100, 0));
                $cartItem->setVat(100);
                $cartItem->setVatIncluded(TRUE);
            ...
            if (abs($grandTotalCorrection) > 0) {
                $cartItem = $cart->addItem(\cardgate\api\Item::TYPE_CORRECTION, 'cg-correction', 'Correction', 1, $grandTotalCorrection);
                $cartItem->setVat(0);
                $cartItem->setVatIncluded(TRUE);
